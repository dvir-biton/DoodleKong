package com.fylora.data

import com.fylora.data.models.*
import com.fylora.gson
import com.fylora.server
import com.fylora.util.getRandomWords
import com.fylora.util.matchesWord
import com.fylora.util.transformToUnderscore
import com.fylora.util.words
...
class Room(
    ...
    suspend fun addPlayer(clientId: String, username: String, socket: WebSocketSession): Player {
        ...
        val announcement = Announcement(
            message = "heyyy! $username joined the party!",
    ...
    fun removePlayer(clientId: String) {
        ...
        val announcement = Announcement(
            "noooo, ${player.username} left the party :(",
    ...
    private fun newRound() {
        ...
        currentWords = getRandomWords(WORDS_AMOUNT)
        ...
        if(isGuessCorrect(message)) {
            ...
            if (isRoundOver) {
                val roundOverAnnouncement = Announcement(
                    message = "GGs, Everybody guessed the word!\nNew round will start soon..",
                    timestamp = System.currentTimeMillis(),
        ...
        word?.let { curWord ->
            drawingPlayer?.let { drawingPlayer ->
                val gameState = GameState(
                    drawingPlayer = drawingPlayer.username,
                    word = if(player.isDrawing || phase == Phase.SHOW_WORD) curWord
                    else curWord.transformToUnderscore()
                )
    ...
    companion object {
        ...
        const val WORDS_AMOUNT = 3

        const val DELAY_WAITING_FOR_START_TO_NEW_ROUND = 25000L
        const val DELAY_NEW_ROUND_TO_GAME_RUNNING = 30000L
        const val DELAY_GAME_RUNNING_TO_SHOW_WORD = 60000L
        const val DELAY_SHOW_WORD_TO_NEW_ROUND = 20000L

        const val PENALTY_NOBODY_GUESSED_IT = 300
        const val GUESS_SCORE_DEFAULT = 200
        const val GUESS_SCORE_PERCENTAGE_MULTIPLIER = 100
        const val GUESS_SCORE_FOR_DRAWING_PLAYER = 300
    }
}